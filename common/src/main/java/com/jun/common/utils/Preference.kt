...
class Preference<T>(val name: String, val default: T) : ReadWriteProperty<Any?, T> {
    ...
    companion object {
        ...
        public fun init(context: Context, fileName: String) {
    ...
    private val prefs: MMKV = MMKVUtils.mmkv!!
    ...
    private fun <A> findPreference(name: String, default: A): A = with(prefs) {
        val res: Any = when (default) {
            ...
            else -> getString(name, serialize(default))?.let(this@Preference::deSerialization)
        }!!
        res as A
    }
